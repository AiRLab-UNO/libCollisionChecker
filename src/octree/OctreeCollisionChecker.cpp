...
namespace octree {
    ...
    bool OctreeCollisionChecker::isCollision(const std::vector<Eigen::VectorXd> &trajectory) const {
        for (const auto &point: trajectory) {
            ...
            if (point[0] < pimpl_->boundary[0] || point[0] > pimpl_->boundary[3] ||
                point[1] < pimpl_->boundary[1] || point[1] > pimpl_->boundary[4] ||
                point[2] < pimpl_->boundary[2] || point[2] > pimpl_->boundary[5]) {
                return true;
            }
